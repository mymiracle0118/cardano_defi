...
const Opened = () => {
  ...
  const getInfo = async () => {
    ...
    const starttime = `${moment.utc(parseInt(presaleinfo.presale_start)*1000).format('Do of MMM, h A')} UTC`
    const endtime = `${moment.utc(parseInt(presaleinfo.presale_end)*1000).format('Do of MMM, h A')} UTC`
    setPresaleInfo([
      {id: "Token Rate:", val: 1/presaleinfo.token_rate},
      ...
      {id: "Presale Start:", val: starttime},
      {id: "Presale End:", val: endtime},
    ])
    ...
    let sale_supply = ethers.utils.formatUnits(tokeninfoarr.totalsupply, tokeninfoarr.decimal) / 100 * 7;
    setTokenInfo([
  ...
  const Withdraw = async () => {
    ...
    try {
      await presalecontract.base_withdraw()
  ...
  const Claim = async () => {
    ...
    try {
      await presalecontract.token_withdraw()
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <CardContent>
              ...
              {presaleState === "Active" && (
                <Fragment>
                  <Typography variant='caption' display="block" sx={{fontWeight: 700, mb: 1}}>Buy JS Token</Typography>
                  <TextField
                    type="number"
                    id="amountToBuy" 
                    label="Amount to Buy" 
